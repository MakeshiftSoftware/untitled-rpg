...
func main() {
	...
	server := server.NewServer(config.Port, accountService, authService)
...
func dbConnect(config config) *sqlx.DB {
	...
	if err := db.QueryRow("SELECT version()").Scan(&dbVersion); err != nil {
		log.Fatal().Err(err).Msg("Failed to query database version")
	}
	log.Info().Str("databaseVersion", dbVersion).Send()
	return db
}
